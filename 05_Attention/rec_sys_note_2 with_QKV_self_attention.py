...
def self_attention_mechanism(input_vectors):
    """
    实现自注意力机制。
    ...
    返回:
    np.ndarray: 形状为 (num_items, num_features) 的自注意力加权结果
    """
    # 使用输入向量作为查询(Q)、键(K)和值(V)
    query = key = value = input_vectors
...
def calculate_interest_scores(user_vectors, movie_vectors):
    """
    计算所有用户对所有电影的兴趣得分。
    ...
    """
    # 应用自注意力机制到用户向量
    user_attention = self_attention_mechanism(user_vectors.values)
    
    # 应用自注意力机制到电影向量
    movie_attention = self_attention_mechanism(movie_vectors.values)
